...
<div class="container p-5 my-5 border" >
	<form action="${path}/board/memo_boardinsert" method="post">
		...
		<div class="input-group mb-3">
			<div class="input-group-prepend">
			등 록 자 명 : <input name="regist_name" value="${param.user_name}" readonly/>	
			※ 등록자명은 수정이 불가능합니다.
		</div> 
			</div>
		</td></tr>
		<tr><td> 
		<div>
			제&nbsp;&nbsp;목 : <input name="title" value="${param.title}" placeholder="제목을 입력하세요(한글기준 50자 이내)"
					style="width:530px;" />	
		</div><br>
		</td></tr>
		<tr><td> 
		<div class="input-group mb-3">
			<div class="input-group-prepend">
			공 지 여 부 : <input type="radio" name="notice_yn" id="Y" value="Y"/>공지 등록
					   <input type="radio" name="notice_yn" id="N" value="N"/>공지 해제
		</div> 
			</div>
		</td></tr>
		<tr><td> 
		<div class="input-group mb-3">
			<div class="input-group-prepend">
			작 성 일 자 : <input type="date" name="regist_date" value="${param.regist_date}" />	
		</div>
			</div>
		</td></tr>
		<tr><td>  
...
<div>&nbsp;</div>		
		<div>
			<button class="btn btn-success" type="button" onclick="goinsert()">저장</button>
