	...
	<div style="float:right;">
		<div>
			<c:if test="${mem.user_name ne null}"><br>
				...
				<input type="button" class="btn btn-warning btn-sm" value="정보수정" onclick="gomemberupt()"/>
				<input type="button" class="btn btn-warning btn-sm" value="로그아웃" onclick="gologout()"/>
					<br>
					<span style="color : #40E0B5; font-size:x-large;">${mem.user_name}님 환영합니다.</span><br>
						<c:if test="${mem.user_status eq 'MASTER' || mem.user_status eq 'ADMIN'}">
							<input type="button" class="btn btn-primary btn-sm" value="회원관리 리스트" onclick="gomemberlist()"/>
						</c:if>
			</c:if>
		</div>
...
<div>&nbsp;</div>
	...
	<button type="submit" value="검색" class="btn btn-primary btn-sm">검색</button>
		...
		<div style="float:right;">  
			<input type="button" class="btn btn-success btn-sm" value="게시글 등록" onclick="gobaordinsert(${mem.user_no})"/>&nbsp;
		</div>
	</form>
...
<div class="container-responsive-xxl">
	...
	<tbody>
	    <c:forEach var="ml" items="${mlist}" >
			<tr class="colored" style="cursor: pointer;" onclick="goDetail(${ml.no})">
				 <td onclick="event.cancelBubble=true"><input name="RowCheck" class="check" type="checkbox" value="${ml.no}"></td>
				 <td>${ml.cnt}</td>
				 <td>	    		 		
	    		 	<c:if test="${ml.notice_yn eq 'Y'}">
	    		 		<img src="${pageContext.request.contextPath}/resources/img/notice.png" width="20px" height="20px"/>
	    		 	</c:if>
	    		 ${ml.title}</td>
				 <td>
				 	<c:if test="${ml.content_reply ne null}">
	    		 		<img src="${pageContext.request.contextPath}/resources/img/reply.png" width="20px" height="20px"/>
	    		 	</c:if>
				 ${ml.content}
				 </td>
				 <td>
				 	<fmt:formatDate value="${ml.write_date}" pattern ="yyyy-MM-dd"/>
				 </td>
				 <td>
					 <fmt:formatDate value="${ml.regist_date}" pattern ="yyyy-MM-dd"/>
				 </td>
				 <td>
					 <fmt:formatDate value="${ml.update_date}" pattern ="yyyy-MM-dd"/>
				 </td>
	...
	<form action="${path}/board/memo_boarddetail" id="detailform2" method="post">
		<input type="hidden" name="no" id="no" value="${ml.no}"/>
		<input type="hidden" name="user_name" id="user_name" value="${mem.user_name}"/>
	</form>
    	</c:forEach>	
	</tbody>
	</table>
...
<div>
	<ul class="pagination justify-content-center" style="margin:20px 0">
	  <li class="page-item">
	  	<a class="page-link" href="javascript:goPage(${memo_boardsch.startBlock}-1)">이전</a></li>
		  <c:forEach var="cnt" begin="${memo_boardsch.startBlock}" end="${memo_boardsch.endBlock}">
		  	<li class="page-item ${memo_boardsch.curPage==cnt?'active':''}">
		  			<a class="page-link" href="javascript:goPage(${cnt})">${cnt}</a></li>
		  </c:forEach>
	  <li class="page-item"><a class="page-link" href="javascript:goPage(${memo_boardsch.endBlock}+1)">다음</a></li>
	</ul> 
</div>	
